...
{
    [CustomAuthorize]
    public class EmployeeProjectPlannerController : Controller
    {
        #region Constant
        ...
        #region Index
        public ActionResult searchByProject(int YearId, int EmployeeId, int HolidayCountryId,int projectId)
        {
            ...
            if (projectId != 0)
            {
                ...
                var scheduling = _employeeProjectPlannerMethod.getAllSchedulingLeves().Where(x => x.EmployeeId == EmployeeId && x.ProjectId == projectId).ToList();
                var timeSheet = _employeeProjectPlannerMethod.getAllTimeSheet().Where(x => x.EmployeeId == EmployeeId).ToList();
                var travelLeave = _employeeProjectPlannerMethod.getAllTravelLeave().Where(x => x.EmployeeId == EmployeeId && x.Project == projectId).ToList();
                var upliftList = _employeeProjectPlannerMethod.getAllUplift().Where(x => x.EmployeeId == EmployeeId && x.ProjectId == projectId).ToList();
                yearModel.yearId = YearId;
                int TotalMonths = 12;
                for (int i = 1; i <= TotalMonths; i++)
                {
                    EmployeeProjectPlannerMonthViewModel monthModel = new EmployeeProjectPlannerMonthViewModel();
                    monthModel.monthId = i;
                    monthModel.yearId = YearId;
                    ...
                    for (int j = 1; j <= totalDays; j++)
                    {
                        ...
                        var travelLeaveTaken = travelLeave.Where(x => date.Date >= x.StartDate.Value.Date && date.Date <= x.EndDate.Value.Date && x.Project == projectId).FirstOrDefault();
                        ...
                        var schedulingTaken = scheduling.Where(x => date.Date >= x.StartDate.Value.Date && date.Date <= x.EndDate.Value.Date && x.ProjectId == projectId).FirstOrDefault();
                        if (schedulingTaken != null)
                        {
                            dayModel.ScheduleId = schedulingTaken.Id;
                            dayModel.isSchedulingLeaveTaken = true;
                        }
                        var UpliftTaken = upliftList.Where(x => date.Date >= x.Date.Date && date.Date <= x.Date.Date && x.ProjectId == projectId).FirstOrDefault();
                        if (UpliftTaken != null)
                        {
        ...
        public ActionResult searchByYear(int YearId, int EmployeeId, int HolidayCountryId)
        {
            EmployeeProjectPlannerYearViewModel yearModel = new EmployeeProjectPlannerYearViewModel();
            if (YearId != 0)
            {
        ...
        public ActionResult Index(int EmployeeId)
        {
            ...
            var List = _db.AspNetUsers.Where(x => x.SSOID.Contains("C") && x.Id==userID && x.Archived == false).ToList();
            if(List.Count>0)
            {
                    if (List.FirstOrDefault().SSOID.StartsWith("C"))
                {
                    model.cflag = 1;
                }
            }           
            ...
            model.ProjectbookDays = _employeeProjectPlannerMethod.GetProjectbookDaysByEmployeeId(EmployeeId);
            model.ProjectRemainingDays = _employeeProjectPlannerMethod.GetProjectRemainingDaysByEmployeeId(EmployeeId);
            model.TimeSheetbookDays = _employeeProjectPlannerMethod.GetTimeSheetbookDaysByEmployeeId(EmployeeId);
            model.TimeSheetRemainingDays = _employeeProjectPlannerMethod.GetTimeSheetRemainingDaysByEmployeeId(EmployeeId);
            model.TravelDays = _employeeProjectPlannerMethod.GetTravelDaysByEmployeeId(EmployeeId);
            model.TravelWorkingDays = _employeeProjectPlannerMethod.GetTravelWorkingDaysByEmployeeId(EmployeeId);
            model.UpliftbookDays = _employeeProjectPlannerMethod.GetUpliftbookDaysByEmployeeId(EmployeeId);
            model.UpliftRemainingDays = _employeePlannerMethod.GetAllWorkingDaysByEmployeeId(EmployeeId);
          ...
          //  var CountryList = _holidayNAbsenceMethod.getAllCountryList();
            ...
            model.ListOfYear = listOfKeyValue;
            return View(model);
        }
        ...
        public ActionResult AddEdit_Scheduling(int Id, int Date, int Month, int Year, int EmpId)
        {
            ...
            //model.customerName = _employeeMethod.getEmpName(EmpId);
