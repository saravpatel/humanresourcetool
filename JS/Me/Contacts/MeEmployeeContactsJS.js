...
$('#tableDiv').on('click', '.dataTr', function () {
    if ($(this).hasClass('dataTr')) {
        $('#EmergencyContactTable tbody').find('tr.selected').removeClass('selected');
...
$("#page_content_inner").on('click', '#SaveContactRecord', function () {
    ...
    var IBAN_Number = $("#page_content").find('#IBAN_Number').val();
...
$("#UpdateContacts").on('click', '#findAddress', function () {
    debugger;
    var isError = false;
    // var hiddenId = $("#tableDivTraining").find("#hidden-Id").val();
